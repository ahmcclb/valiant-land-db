        ...
        try:
            ...
            for deletion in pending_deletions:
                ...
                id_columns = {
                    ...
                    'property_photos': 'photo_id',  # FIX: Add photos
                    'property_documents': 'doc_id'   # FIX: Add documents
                }
            ...
            try:
                ...
                # Handle deletions for all tables
                id_columns = {
                    'properties': 'p_id',
                    'property_links': 'link_id',
                    'property_photos': 'photo_id',  # FIX: Add photos
            ...
            for cloud_owner in cloud_owners.data:
                ...
                # Filter out sync metadata to prevent duplicate column assignments
                clean_columns = {k: v for k, v in cloud_owner.items() 
                               if k not in ('sync_status', 'last_sync_at', 'sync_source')}
                columns = list(clean_columns.keys())
                values = [clean_columns[col] for col in columns]
            ...
            for cloud_prop in cloud_props.data:
                ...
                # FIX: Filter out sync metadata columns from cloud data
                clean_columns = {k: v for k, v in cloud_prop.items() 
                               if k not in ('sync_status', 'last_sync_at', 'sync_source')}
            ...
            try:
                ...
                for link in cloud_links.data:
                    try:
                        # FIX: Ensure datetime fields are properly handled
            ...
            # FIX: PHASE 4 - Pull property_photos and property_documents from cloud
            try:
